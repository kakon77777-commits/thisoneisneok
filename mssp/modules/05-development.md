...
**而第一次執行抓到的是錨點自己的缺陷。** `005` 贏了每一個計分的軸，我的第一反應是「所有軸都只是在追大小」——所以加了一節讓錨點量自己的軸獨立性，然後**量測不同意我**：6 對裡只有 1 對過 |rho| ≥ 0.8，而那一對是代數上必然的（`ratio = total ÷ worst`）；`total_source_lines` 與 `sms_share_pct` 的相關是 **0**。
