...
實作：[001 號範例](/html/mssp/001-task-runner.html)。

## 模式 2：註冊表，讓「只載入一個」成為可能

沒有註冊表，入口檔案必須直接 import 每一個模組。這會讓「建立一個只有一個 TMS 的系統」在物理上不可能——於是孤島測試也不可能。

註冊表的價值不在於它做了什麼，而在於它**讓一件事變得可以被構造**。這是 SMS 的典型長相：本身很小，但拿掉之後某一整類驗證會消失。
...
`build-mssp.mjs` 有一項檢查禁止 TMS 引用兄弟 TMS。第一版的邏輯把「同一個目錄內」視為模組內部，所以放行。但 `TMS/reporters/` 裡放的是**兩個不同的 TMS**——同目錄恰好就是最該擋的情況。
