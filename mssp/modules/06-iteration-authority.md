...
**三、有一條可以跑的驗收條件。** §57-F：移除 Dynamic MSSP Profile 之後，治理核心仍然要能運作。這條不是宣言，是會失敗的檢查——而目前 [EveMiss FPL](/research/evemiss-fpl.html) 過不了它：五個集合的名字在原始碼裡出現 87 次，其中 45 次在型別檢查器裡，也就是說那八條規則檢查的不是架構治理，是五個特定字串。這件事已經記在[開發日誌](/html/mssp/modules/log.html)。
